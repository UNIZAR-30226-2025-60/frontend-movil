...
import { StyleSheet, ScrollView, Text, TextInput, Switch, Image, View, TouchableOpacity, Alert, FlatList, TouchableWithoutFeedback } from 'react-native';
...
export default function DetallesLibro({ route, correoUsuario }) {
   ...
   const mostrarDropdown = () => {
      if (modalOrdenarVisible) {
         setModalOrdenarVisible(false); // Si ya está visible, lo ocultamos
         return;
      }
   ...
   return (
      <ScrollView
         onScrollBeginDrag={() => setModalOrdenarVisible(false)}
         contentContainerStyle={[stylesGeneral.container, { backgroundColor: colors.background }]}
      >
