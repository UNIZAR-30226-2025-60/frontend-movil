...
import { StyleSheet, Text, View, Alert, TextInput, TouchableOpacity, Switch, TouchableWithoutFeedback } from 'react-native';
...
export default function ListadoPreguntasForo({ correoUsuario }) {
   ...
   const mostrarDropdown = () => {
      ...
      if (botonOrdenarRef.current) {
         botonOrdenarRef.current.measure((x, y, width, height, pageX, pageY) => {
            const offset = correoUsuario ? 100 : 3;
            setBotonOrdenarLayout({ x: pageX, y: y + height + offset, width: 200 });
   ...
   useFocusEffect(
      React.useCallback(() => {
         // Resetea los estados a sus valores por defecto al volver a la pantalla
         setVerMisPreguntas(false);
         setOrdenSeleccionado('ninguno');
         setExpandedQuestion({});
         // Si deseas resetear también los arrays de preguntas ordenadas, podrías restablecerlos
         setTodasPreguntasOrdenadas(todasPreguntas);
         setMisPreguntasOrdenadas(misPreguntas);
         // El callback no necesita devolver nada
         return () => { };
      }, [correoUsuario, todasPreguntas, misPreguntas])
   );
   ...
   return (
      <TouchableWithoutFeedback onPress={cerrarDropdown}>
